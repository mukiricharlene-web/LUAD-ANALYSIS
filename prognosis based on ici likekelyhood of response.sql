...
FROM final_stratification
immune_stratification AS (
SELECT *,
    CASE
        WHEN cd8a_tile = 3 AND ifng_tile = 3 THEN 'functional_immunity'
        WHEN cd8a_tile = 3 AND ifng_tile = 1 THEN 'inactive_immunity'
        WHEN cd8a_tile = 1 AND ifng_tile = 1 THEN 'immune desert'
        ELSE 'Non_canonical_immunity'
    END AS immune_prescence,
    CASE 
        WHEN pdcd1_tile = 3 AND lag3_tile = 3 THEN 'tcell_exhausted'
        WHEN cd274_tile = 3 THEN 'tumour_pdl1_high'
        WHEN pdcd1_tile = 1 AND lag3_tile = 1 THEN 'non_exhausted'
    ELSE 'intermediate'
...
SELECT
    ...
    survival_status,
    CASE 
        WHEN immune_prescence = 'functional_immunity' AND immune_phenotype = 'tcell_exhausted'
        THEN 'high_likelihood_response'
        WHEN immune_prescence = 'inactive_immunity' AND immune_phenotype IN ('tcell_exhausted', 'tumour_pdl1_high')
        THEN 'potential_response'
        WHEN immune_prescence = 'immune_desert' THEN 'low_likelihood_response'
        WHEN immune_prescence = 'functional_immunity' AND immune_phenotype = 'non_exhausted'
        THEN 'low_incremental_benefit'
        ELSE 'uncertain'
    END AS ici_response_stratification
FROM
    immune_stratification
ORDER BY
    survival_time DESC,
